...
./output/<video-id>/
    ...
    images/                     # 留空；关键帧 + 网图插入在 v0.4 backlog
...
- **独立 CLI 模式**：`OPENAI_API_KEY` / `ANTHROPIC_API_KEY` / `OPENROUTER_API_KEY` 任选一。Ollama 不需要 key —— 本地 `ollama serve` 跑起来即可（或设 `OLLAMA_HOST`）。
...
内置五种风格（作为 package data 打包随 pip 带上）：

- `default` — 通用博客 / newsletter 语气
- `technical` — 技术向、面向开发者
- `newsletter` — 对话式、钩子句开场、scannable 段落
- `summary` — 紧凑 300-500 词摘要，bullet 为主
- `interview` — 保留逐字引用，适合播客 / 访谈
...
- **v0.2** ✓ — 正确性 / UX 打磨：>25 MB 音频自动分段、UUID 分隔符防 prompt 注入、浏览器 cookie 检测加严、slug 长度上限。
- **v0.3** ✓ — OpenRouter + Ollama providers（共 4 个）、新增 3 个 style（`newsletter` / `summary` / `interview`）、GitHub Actions CI。
- **v0.4** — 多模型 judge loop、B 站一等公民支持、关键帧 + 网图插入。
- **v0.5+** — Codex / Cursor / 其他 agent skill 适配器、可选 HTTP API。
...
`ROADMAP.md` 有 v0.4 backlog，都是具体、小范围的任务。欢迎 PR。
